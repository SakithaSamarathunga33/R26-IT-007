...
import {
  SpeechLevelProgress,
  clearLevelPredictions,
  fetchLevelProgress,
  isLevelUnlocked,
} from "../../services/speechLevelService";
...
export default function SpeechLevelsScreen({ navigation }: Props) {
  ...
  const startLevel = async (level: SpeechLevel) => {
    ...
    if (uid) {
      if (level.id === 1) {
        await clearModulePredictions(uid, "speech").catch(() => {});
      } else {
        await clearLevelPredictions(uid, level.id).catch(() => {});
      }
    }
    const firstIndex = taskIndicesForLevel(level.id)[0];
    navigation.navigate("SpeechActivity", { taskIndex: firstIndex });
  };
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.progressTop}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.progressSub}>
                {completedCount} of {SPEECH_LEVELS.length} levels complete
              </Text>
            </View>
            <Text style={styles.progressPct}>
              {Math.round((completedCount / SPEECH_LEVELS.length) * 100)}%
            </Text>
          </View>
          <View style={styles.progressTrack}>
            <LinearGradient
              colors={["#3B72F6", "#2563EB"]}
              style={[styles.progressFill, { width: `${(completedCount / SPEECH_LEVELS.length) * 100}%` }]}
        ...
        {loading ? (
          ...
          SPEECH_LEVELS.map((level, i) => {
            ...
            const count = levelTaskCount(level.id);
        ...
        <View style={styles.noteCard}>
          ...
          <Text style={styles.noteText}>
            Levels get harder as you go. Each one starts with the app saying the word — listen first, then say it back.
          </Text>
        </View>
